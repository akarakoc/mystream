...
class posttypeList(forms.Form):
    def __init__(self, *args, **kwargs):
        ...
        self.fields['PosttypeEntry'] = forms.ModelChoiceField(queryset=CommList.datatypes_set.all().order_by('name'),
                                                              label='', to_field_name="name")
...
class searchList(forms.Form):
    def __init__(self, *args, **kwargs):
        ...
        CommList = Communities.objects.filter(communityHash=cmHash)[0]
        self.fields['searchEntry'] = forms.ModelChoiceField(queryset=CommList.datatypefields_set.all().order_by('name'),
                                                            label='', to_field_name="name")
